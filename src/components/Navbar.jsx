...
export default function Navbar() {
  ...
  useEffect(() => {
    ...
    if (!menu) return;
    if (mobileOpen) {
      gsap.fromTo(menu, { opacity: 0, scale: 0.95, y: -10 }, { opacity: 1, scale: 1, y: 0, duration: 0.3, ease: "power3.out" });
      gsap.fromTo(menu.querySelectorAll(".mobile-link"), { opacity: 0, y: 12 }, { opacity: 1, y: 0, duration: 0.35, stagger: 0.05, ease: "power3.out", delay: 0.08 });
    }
  }, [mobileOpen]);
        ...
        <div className="relative flex justify-between items-center h-12 w-full">
          <Link href="/" onClick={handleLogoClick} className="flex items-center group">
            <Image
              ...
              width={120}
              height={48}
              className={`h-12 w-auto object-contain transition-all duration-500 group-hover:scale-105 ${isMounted && scrolled ? "brightness-0 invert" : ""}`}
              priority
            />
          </Link>
            ...
            {links.map(({ labelKey, href }) => {
              ...
              return (
                <Link key={label} href={href} onClick={handleLinkClick} className={cls}>
                  {label}
                  <span className={`absolute bottom-0 left-1/2 -translate-x-1/2 h-[2px] bg-accent transition-all duration-300 ${isActive ? "w-[60%]" : "w-0 group-hover/item:w-[60%]"}`} />
                </Link>
              );
            })}
          </div>
          ...
          <div className="hidden md:flex items-center gap-4">
            <div className="relative" ref={langDropdownRef}>
              ...
              >
                ...
                <span>{locale}</span>
                <ChevronDown className={`w-3 h-3 transition-transform duration-300 ${langOpen ? "rotate-180" : ""}`} strokeWidth={2.5} aria-hidden="true" />
              </button>
